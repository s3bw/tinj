...
// LineAggregator turns a JSON input into metrics
...
// Print aggregation after reading line
func (l *LineAggregator) Print(line []byte) {
	...
	// Magic character to move to the start of line above
	for i := 0; i < removeLine; i++ {
		fmt.Print("\033[F")
	}
}
...
func AggregateStdin(args, sep string) {
	...
	for range lineAggregator.Metrics {
		fmt.Print("\n")
	}
}
